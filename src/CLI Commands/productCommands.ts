...
export async function productCLI(state: State, ...args: string[]): Promise<void> {
...
async function findProduct(
  ...
  if (categoryFlag && typeof categoryFlag === 'string') {
    ...
    for (const p of products) {
      console.log(`- ${p.name} | $${p.price ?? 'N/A'} | Qty: ${p.quantity} | ${p.manufacturer.name}`);
    }
    return;
  }
  ...
  if (manufacturerFlag && typeof manufacturerFlag === 'string') {
    ...
    const result = await getProductsByManufacturer({ manufacturerId: manufacturer.id });
    ...
    for (const p of products) {
      console.log(`- ${p.name} | SKU: ${p.sku} | $${p.price ?? 'N/A'} | Qty: ${p.quantity} | ${p.category.name}`);
    }
    return;
  }
...
async function addProductInteractive(state: State, categoryName: string): Promise<void> {
  const { readline } = state;

  const categoryId = await getCategoryUUIDByName(categoryName);
  if (!categoryId) {
    console.log(`Category "${categoryName}" not found. Run "category ls" to see available categories.`);
    return;
  }
  ...
  if (!sku) { console.log('SKU is required.'); return; }

  const description = (await prompt(readline, 'Description: ')).trim();
  if (!description) { console.log('Description is required.'); return; }

  const colorRaw = (await prompt(readline, 'Color (optional, press Enter to skip): ')).trim();
  const color = colorRaw || null;

  const msrpRaw = (await prompt(readline, 'MSRP ($): ')).trim();
  const msrp = parseFloat(msrpRaw);
  if (isNaN(msrp) || msrp <= 0) { console.log('Invalid MSRP.'); return; }

  const priceRaw = (await prompt(readline, 'Sale price ($, optional, press Enter to skip): ')).trim();
  const price = priceRaw ? parseFloat(priceRaw) : null;
  if (priceRaw && (isNaN(price!) || price! <= 0)) { console.log('Invalid price.'); return; }

  const quantityRaw = (await prompt(readline, 'Quantity: ')).trim();
  const quantity = parseInt(quantityRaw, 10);
  if (isNaN(quantity) || quantity < 0) { console.log('Invalid quantity.'); return; }

  const manufacturerName = (await prompt(readline, 'Manufacturer name: ')).trim();
...
async function promptSpecsByCategory(
  ...
  if (cat === 'cpus' || cat === 'cpu') {
    ...
    const baseClockGHz = parseFloat((await prompt(readline, 'Base clock (GHz): ')).trim());
    const boostRaw = (await prompt(readline, 'Boost clock (GHz, optional): ')).trim();
    const boostClockGHz = boostRaw ? parseFloat(boostRaw) : null;
    const tdp = parseInt((await prompt(readline, 'TDP (W): ')).trim(), 10);
    const socketType = (await prompt(readline, 'Socket type (e.g. AM5, LGA1700): ')).trim();
    const igRaw = (await prompt(readline, 'Integrated graphics? (y/n): ')).trim().toLowerCase();
    ...
    const vars = createCpuSpecsSchema.parse({ productId, cores, threads, baseClockGHz, boostClockGHz, tdp, socketType, integratedGraphics, cacheMB });
    await addCPUSpecs(vars);
    console.log('CPU specs saved.');

  } else if (cat === 'gpus' || cat === 'gpu') {
    const chipset = (await prompt(readline, 'Chipset (e.g. RTX 4080): ')).trim();
    const vramGB = parseInt((await prompt(readline, 'VRAM (GB): ')).trim(), 10);
    const vramType = (await prompt(readline, 'VRAM type (e.g. GDDR6X): ')).trim();
    const coreRaw = (await prompt(readline, 'Core count (optional): ')).trim();
    const coreCount = coreRaw ? parseInt(coreRaw, 10) : null;
    const baseRaw = (await prompt(readline, 'Base clock (MHz, optional): ')).trim();
    const baseClockMHz = baseRaw ? parseInt(baseRaw, 10) : null;
    const boostRaw = (await prompt(readline, 'Boost clock (MHz, optional): ')).trim();
    ...
    const powerConnectors = (await prompt(readline, 'Power connectors (e.g. 2x8-pin, optional): ')).trim() || null;

    const vars = createGpuSpecsSchema.parse({ productId, chipset, vramGB, vramType, coreCount, baseClockMHz, boostClockMHz, tdp, lengthMM, powerConnectors });
    await addGPUSpecs(vars);
    console.log('GPU specs saved.');

  } else if (cat === 'storage' || cat === 'ssds' || cat === 'hdds') {
    const storageType = (await prompt(readline, 'Storage type (SSD/HDD/NVMe): ')).trim();
    const capacityGB = parseInt((await prompt(readline, 'Capacity (GB): ')).trim(), 10);
    const iface = (await prompt(readline, 'Interface (e.g. SATA, PCIe 4.0): ')).trim();
    const formFactor = (await prompt(readline, 'Form factor (e.g. 2.5", M.2): ')).trim();
    const readRaw = (await prompt(readline, 'Read speed (MB/s, optional): ')).trim();
    const readSpeedMBps = readRaw ? parseInt(readRaw, 10) : null;
    const writeRaw = (await prompt(readline, 'Write speed (MB/s, optional): ')).trim();
    const writeSpeedMBps = writeRaw ? parseInt(writeRaw, 10) : null;

    const vars = createStorageSpecsSchema.parse({ productId, storageType, capacityGB, interface: iface, formFactor, readSpeedMBps, writeSpeedMBps });
    ...
    const efficiencyRating = (await prompt(readline, 'Efficiency rating (e.g. 80+ Gold): ')).trim();
    const modular = (await prompt(readline, 'Modular type (Full/Semi/Non): ')).trim();
    const formFactor = (await prompt(readline, 'Form factor (e.g. ATX): ')).trim();

    const vars = createPsuSpecsSchema.parse({ productId, wattage, efficiencyRating, modular, formFactor });
    await addPSUSpecs(vars);
    console.log('PSU specs saved.');

  } else if (cat === 'cpu coolers' || cat === 'coolers' || cat === 'cpu-coolers') {
    const coolerType = (await prompt(readline, 'Cooler type (Air/AIO): ')).trim();
    const radRaw = (await prompt(readline, 'Radiator size (MM, optional, AIO only): ')).trim();
    ...
    const maxTDP = parseInt((await prompt(readline, 'Max TDP (W): ')).trim(), 10);
    const socketCompatibility = (await prompt(readline, 'Socket compatibility (e.g. AM5,LGA1700): ')).trim();
    const heightMM = parseInt((await prompt(readline, 'Height (MM): ')).trim(), 10);

    const vars = createCpuCoolerSpecsSchema.parse({ productId, coolerType, radiatorSizeMM, fanSizeMM, maxTDP, socketCompatibility, heightMM });
    await addCPUCoolerSpecs(vars);
    console.log('CPU cooler specs saved.');

  } else if (cat === 'cases' || cat === 'case') {
    const formFactor = (await prompt(readline, 'Form factor (e.g. ATX Mid-Tower): ')).trim();
    const moboSupport = (await prompt(readline, 'Motherboard support (e.g. ATX,mATX,ITX): ')).trim();
    const gpuRaw = (await prompt(readline, 'Max GPU length (MM, optional): ')).trim();
    const maxGPULengthMM = gpuRaw ? parseInt(gpuRaw, 10) : null;
    const coolerRaw = (await prompt(readline, 'Max CPU cooler height (MM, optional): ')).trim();
    const maxCPUCoolerHeightMM = coolerRaw ? parseInt(coolerRaw, 10) : null;
    const psuRaw = (await prompt(readline, 'Max PSU length (MM, optional): ')).trim();
    const maxPSULengthMM = psuRaw ? parseInt(psuRaw, 10) : null;
    const twoHalfRaw = (await prompt(readline, '2.5" drive slots (optional): ')).trim();
    const driveSlotsTwoHalf = twoHalfRaw ? parseInt(twoHalfRaw, 10) : null;
    const threeHalfRaw = (await prompt(readline, '3.5" drive slots (optional): ')).trim();
    const driveSlotsThreeHalf = threeHalfRaw ? parseInt(threeHalfRaw, 10) : null;
    const fanRaw = (await prompt(readline, 'Fan slots (optional): ')).trim();
    const fanSlots = fanRaw ? parseInt(fanRaw, 10) : null;
    const radiatorSupport = (await prompt(readline, 'Radiator support (e.g. 240mm, 360mm, optional): ')).trim() || null;

    const vars = createCaseSpecsSchema.parse({ productId, formFactor, moboSupport, maxGPULengthMM, maxCPUCoolerHeightMM, maxPSULengthMM, driveSlotsTwoHalf, driveSlotsThreeHalf, fanSlots, radiatorSupport });
    await addCaseSpecs(vars);
    console.log('Case specs saved.');

  } else if (cat === 'ram' || cat === 'memory') {
    const memoryType = (await prompt(readline, 'Memory type (e.g. DDR5): ')).trim();
    const speedMHz = parseInt((await prompt(readline, 'Speed (MHz): ')).trim(), 10);
    const capacityGB = parseInt((await prompt(readline, 'Capacity (GB): ')).trim(), 10);
    const modules = parseInt((await prompt(readline, 'Number of modules: ')).trim(), 10);
    ...
    const vars = createRamSpecsSchema.parse({ productId, memoryType, speedMHz, capacityGB, modules, casLatency, voltage });
    await addRAMSpecs(vars);
    console.log('RAM specs saved.');

  } else if (cat === 'motherboards' || cat === 'motherboard') {
    const socketType = (await prompt(readline, 'Socket type (e.g. AM5): ')).trim();
    const chipset = (await prompt(readline, 'Chipset (e.g. X670E): ')).trim();
    const formFactor = (await prompt(readline, 'Form factor (e.g. ATX): ')).trim();
    const memoryType = (await prompt(readline, 'Memory type (e.g. DDR5): ')).trim();
    const memorySlots = parseInt((await prompt(readline, 'Memory slots: ')).trim(), 10);
    const maxMemoryGB = parseInt((await prompt(readline, 'Max memory (GB): ')).trim(), 10);
    ...
    const vars = createMotherboardSpecsSchema.parse({ productId, socketType, chipset, formFactor, memoryType, memorySlots, maxMemoryGB, m2Slots, sataSlots, pciSlots });
    await addMotherboardSpecs(vars);
    console.log('Motherboard specs saved.');

  } else {
    console.log(`No spec form available for category "${categoryName}". Product was created without specs.`);
  }
}
...
async function updateProductInteractive(
  ...
  if (idFlag && typeof idFlag === 'string') {
    ...
    const result = await searchProductsByName({ nameQuery: productName, limit: 10 });
    const products = result.data.products;
    ...
    if (products.length === 1) {
      ...
      products.forEach((p, i) => {
        console.log(`  ${i + 1}. ${p.name} (SKU: ${p.sku}) — ${p.manufacturer.name}`);
      });
  ...
  const descRaw = (await prompt(readline, `Description [${existing.description}]: `)).trim();
  const colorRaw = (await prompt(readline, `Color [${existing.color ?? ''}]: `)).trim();
  const priceRaw = (await prompt(readline, `Price [$${existing.price ?? 'N/A'}]: `)).trim();
  const quantityRaw = (await prompt(readline, `Quantity [${existing.quantity}]: `)).trim();
